...
<head>
  ...
  <!-- CSS -->
  <link rel="stylesheet" href="giohangK.css">
</head>
...
<a href="donhang.php" class="order-icon">
  <i class="fa-solid fa-box"></i>
  <span id="order-count"><?= $order_count ?></span>
</a>
    </div>
  </div>
...
<div class="cart-left">
  ...
  <!-- Phần này sẽ chứa tất cả sản phẩm -->
...
<!-- ================= GHI CHÚ CỦA CỬA HÀNG ================= -->
    <div class="order-policy">
      ...
      <p><i class="fa-solid fa-check"></i>
        ...
        Miễn phí giao hàng & lắp đặt tại tất cả quận huyện thuộc TP.HCM, Hà Nội,
        Khu đô thị Ecopark, Biên Hòa và một số khu vực thuộc Bình Dương (*)
      </p>

      <p><i class="fa-solid fa-check"></i>
        Đơn hàng của quý khách sẽ được <strong>giao hàng trong vòng 3 ngày</strong>,
        vui lòng đợi nhân viên tư vấn xác nhận lịch giao hàng trước khi thực hiện chuyển khoản.
      </p>
      ...
      <p><i class="fa-solid fa-check"></i>
        ...
        <strong>Đan Mạch và Nhật Bản</strong>
      </p>
      ...
      <div class="policy-note">
        ...
        <p>(**) Không áp dụng cho các sản phẩm Clearance. Chỉ bảo hành 01 năm
          cho khung ghế, mâm và cần đối với Ghế Văn Phòng</p>
      </div>
    </div>
  </div>
  </div>
...
<script>
        ...
        cart.forEach(item => {
            ...
            div.innerHTML = `
                <div style="display: flex; gap: 15px; align-items: center; padding: 15px 0; border-bottom: 1px solid #eee;">
                    ...
                    <img src="${item.img}" width="90" height="90" style="object-fit: cover; border-radius: 8px;">
                    ...
                    <button onclick="removeItem(${item.id})" 
                            style="background: #6b442d; color: white; border: none; padding: 8px 14px; border-radius: 6px; cursor: pointer; font-size: 14px;">
                        Xóa
                    </button>
                </div>
            `;
    ...
    function updateCartCountHeader() {
        let totalQty = cart.reduce((sum, item) => sum + (parseInt(item.quantity) || 0), 0);
    ...
    function checkout() {
        ...
        // Lưu sản phẩm đã chọn để thanh toán
        localStorage.setItem("buyNow", JSON.stringify(selectedItems));

        // XÓA các sản phẩm đã chọn khỏi giỏ hàng
        cart = cart.filter(item => !selectedIds.includes(parseInt(item.id)));

        // Cập nhật lại localStorage giỏ hàng
        localStorage.setItem("cart_" + currentUserId, JSON.stringify(cart));
...
// ================== KHỞI TẠO ==================
    document.addEventListener("DOMContentLoaded", function() {
        renderCart();                    // Load giỏ hàng khi vào trang
        updateCartCountHeader();
    });
