...
<head>
  ...
  <!-- CSS -->
  <link rel="stylesheet" href="dangky5.css">
</head>
<body>
...
<section class="register-page">
  <div class="register-box">
    ...
    <div class="register-left">
      ...
      <img src="img/anhnen.png" alt="">

      <p>
        Chúng tôi giao hàng nhanh như cách mà bạn<br>
        đăng ký tài khoản.
      </p>
    </div>
...
<div class="register-right">
  ...
  <a href="trangchu.php" class="back"> 
    ← Quay lại trang chủ
  </a>
...
document.getElementById("registerForm").addEventListener("submit", function(e) {
    ...
    if (!phonePattern.test(sdt)) {
        ...
        message.innerText = "Số điện thoại không hợp lệ (phải 10 số, đúng đầu số VN)";
    ...
    .then(res => res.json())
    .then(json => {
        console.log("PHP trả về:", json);

        if (json.status === "success") {
            message.style.color = "green";
            message.innerText = json.message || "Đăng ký thành công!";
            setTimeout(() => window.location.href = "trangchu.php", 1500);
        } 
        else if (json.status === "exists") {
            message.style.color = "red";
            message.innerText = json.message || "Email đã tồn tại!";
        } 
        else if (json.status === "empty") {
            message.style.color = "red";
            message.innerText = json.message || "Thiếu thông tin!";
        } 
        else {
            message.style.color = "red";
            message.innerText = json.message || "Lỗi hệ thống, thử lại sau!";
        }
    })
    .catch(() => {
        message.style.color = "red";
        message.innerText = "Lỗi kết nối hoặc server!";
    })
    .finally(() => {
        button.disabled = false;
        button.innerText = "ĐĂNG KÝ";
